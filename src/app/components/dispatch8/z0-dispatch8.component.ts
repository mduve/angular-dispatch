...
export class Dispatch8Component {
  ...
  // getData(branchdata) {
  //   this.afs.collection('stocks', ref => ref.where("branchId", "==", branchdata).where("status", ">=", "Wait Dispatch").where("status", "<=", "Wait Off Site Tow Dispatch")).valueChanges().subscribe((stocks) => {
  //     this.rows = stocks;
  ...
  getData(branchdata) {
    this.afs.collection('stocks', ref => ref.where("branchId", "==", branchdata)).valueChanges().subscribe((stocks) => {
      this.rows = stocks;
      //create allStocks array with isChecked property
      this.allStocks.push(...stocks);
      this.allStocks.map(function(newprop) {
        newprop.isChecked = false;   
        return newprop;
      });
      //create isChecked property on rows
      // this.rows.map(function(newprop) {
      //   newprop.isChecked = false;   
      //   return newprop;
      // });
  ...
  onSelect({ selected }) {
    ...
    this.selAllStocks.splice(0, this.selected.length);
    this.selAllStocks.push(...selected);
    ...
    //need match or find for stock numbers
    // selected.forEach(function(element) {
    ...
    // console.log(this.selected.length);
    // const validate = <FormArray>this.firstFormGroup.get('firstCtrl') as FormArray;
    // if (this.selected.length > 0){ } else { } 
  }
  ...
  selMarkerOn(i) {
    ...
    this.selected.splice(0, this.selected.length);
    this.selected.push(...this.selAllStocks);
    // console.log(this.selected);
    // console.log(this.selAllStocks);
    // console.log(i);
    
    // this.myCheckboxes.forEach(function(element) {
